...
import { useAuth } from '../context/AuthContext'; // Importing our global Auth state

const Navbar = () => {
  const [isScrolled, setIsScrolled] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const location = useLocation();
  const { user, logout } = useAuth(); // Hooking into the Auth state
  ...
  return (
    <>
      ...
      <header 
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ease-in-out px-4 md:px-8 py-4 ${
          isScrolled ? 'py-3' : 'py-5'
        }`}
      >
        <div 
          className={`max-w-[1400px] mx-auto flex items-center justify-between rounded-full transition-all duration-500 ${
            isScrolled 
              ? 'bg-[#1a1f16]/95 backdrop-blur-md shadow-[0_10px_40px_-10px_rgba(0,0,0,0.3)] py-3 px-6 md:px-8 border border-white/10' 
              : 'bg-[#1a1f16]/80 backdrop-blur-sm py-4 px-6 md:px-8 shadow-lg'
          }`}
        >
          {/* Logo Area */}
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  {link.name}
                  {isActive && (
                    <span className="absolute -bottom-1.5 left-0 w-full h-[2px] bg-[#85B060] rounded-full"></span>
                  )}
                </Link>
              );
            })}
          ...
          <div className="hidden lg:flex items-center gap-5 shrink-0">
            ...
            <div className="flex items-center gap-2 bg-white/10 border border-white/10 rounded-full px-4 py-2 hover:bg-white/20 transition-colors cursor-text">
              ...
              <input 
                ...
                className="bg-transparent border-none outline-none text-white text-xs w-28 placeholder:text-white/50"
              />
            </div>
            ...
            <Link to="/cart" className="relative text-white/90 hover:text-[#85B060] transition-colors">
              ...
              <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-[#85B060] text-white text-[9px] font-black flex items-center justify-center rounded-full border border-[#1a1f16]">
                2
              </span>
            </Link>

            {/* --- NEW DYNAMIC AUTHENTICATION LOGIC --- */}
            {user ? (
              <div className="relative group cursor-pointer ml-1">
                ...
                {/* Dropdown Menu (Dashboard & Logout) */}
          ...
          <button 
            className="lg:hidden text-white p-1 hover:text-[#85B060] transition-colors"
      ...
      {isMobileMenuOpen && (
        <div 
          className="fixed inset-0 bg-[#0a1508]/60 backdrop-blur-sm z-[100] lg:hidden"
      ...
      {/* Mobile Sidebar Menu */}
      <div 
        className={`fixed top-0 right-0 h-full w-[85vw] sm:w-[350px] bg-white z-[110] transform transition-transform duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] flex flex-col shadow-2xl lg:hidden ${
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          ...
          <button 
            onClick={() => setIsMobileMenuOpen(false)}
            className="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center text-gray-500 hover:text-black hover:bg-gray-100 transition-colors"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-6 px-6 flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3 bg-[#F8F9F5] border border-gray-200 rounded-2xl px-4 py-3">
            ...
            <input 
              type="text" 
              placeholder="Search plants..." 
              className="bg-transparent border-none outline-none text-[#1a1f16] text-sm w-full font-medium"
            />
          </div>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link 
                  key={link.name} 
                  to={link.path}
                  className={`text-lg font-extrabold py-3 px-4 rounded-2xl transition-colors ${
                    isActive ? 'bg-[#EAF0E3] text-[#4A6731]' : 'text-[#1a1f16] hover:bg-gray-50'
                  }`}
                >
                  {link.name}
                </Link>
             ...
             <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link to="/cart" className="relative w-[52px] h-[52px] bg-white border border-gray-200 rounded-full flex items-center justify-center text-[#1a1f16] shadow-sm shrink-0">
                    <ShoppingCart size={20} />
                    <span className="absolute top-0 right-0 w-4 h-4 bg-[#85B060] text-white text-[10px] font-black flex items-center justify-center rounded-full border border-white">
                      2
                    </span>
                  </Link>
                </div>
